...
The event mechanism is facilitated by all four major components (UI, Logic, Model, and Storage) of the Architecture.
It supports the additional features of event management, which allows the user to add, edit, delete, find, or select event by utilizing the CLI
commands in the TunedIn application. It also interacts with the newly added calendar graphical interface (refer to the image below).

image::CalendarUI.png[width="500"]

Firstly, for Event mechanism to incorporate TunedIn, the UI component will remain the same as how it deals with the
previous `Addressbook` application. For the Logic component, it will also remain the same, but with a new naming for the parser
(`AddressbookParser` -> `GeneralbookParser`) Reason being is because it will indicate a much clearer meaning that it can process
command for both `Addressbook` and `Eventbook`.

For the Model component, event mechanism will deals with `EventBook`, and faciliated by `ModelManager`.

The following diagram shows the inheritance diagram for `EventBook`:

image::EventModelClassDiagram.png[width="800"]

As you can see from the diagram, `Event` stores `Title`, `Description`, `Location`, and `Datetime as an attribute. `EventBook` consists of a `UniqueEventList`, which stores multiple `Event` objects. ModelManager will facilitate the behaviour of `EventBook`.

The implementation of the faciliation between ModelManager and EventBook is shown below in greater detail. As you can see from the implementation, it shows how the CRUD (Create, Read, Update, Delete) process work for the Model Component of the event mechanism.
...
----
For the Storage component, event mechanism will deals with `EventBookStorage`, and
faciliated by `StorageManager`.

The following diagram shows the inheritance diagram for `EventBookStorage`:

image::EventStorageClassDiagram.png[width="800"]

As you can see from the diagram `EventBookStorage` will do the reading and writing to XML Eventbook file, where all the data is located at.

The implementation of the class `EventBookStorage` is shown below in greater detail. As you can see from the implementation, it shows how the reading and writing process.
...
The following sequence diagram shows how the update event operation works:

image::UpdateEventSequenceDiagram.png[width="800"]

The following sequence diagram shows how the delete event operation works:

image::DeleteEventSequenceDiagram.png[width="800"]

The following activity diagram summarize what happens inside the `EventBook` when a user executes a new `addevent` command:

image::AddEventActivityDiagram.png[width="300"]

The following activity diagram summarize what happens inside the `EventBook` when a user executes a new `editevent` command:

image::UpdateEventActivityDiagram.png[width="300"]

The following activity diagram summarize what happens inside the `EventBook` when a user executes a new `deleteevent` command:

image::DeleteEventActivityDiagram.png[width="300"]

==== Design Considerations

**Aspect:** Implementation of Event mechanism +
**Alternative 1 (current choice):** Creates another different set of similar classess and methods to Addressbook +
**Pros:** Clear distinction between Eventbook and Addressbook, neater architecture. +
**Cons:** Tedious, as the implementation needs to copy and edit from the previous Addressbook application, and easily prone to error due to human carelessness. +
**Alternative 2:** Fuse everything with the Addressbook application without creating separate set of classess and methods +
**Pros:** Easier to implement +
**Cons:** Architecture will be messy as there is no clear distinct line between Eventbook and Addressbook.
...
to export either addressbook or eventbook in a excel format.

Since addressbook format is slightly different from eventbook format, it can't be done through a general exportation as a single person in addressbook can have multiple tagged.

The implementation of exporting addressbook into a csv file is shown below:
...
The implementation of exporting eventbook into a csv file is shown below:
...
The following activity diagram summarize what happens inside the `XmlFileStorage` when a user executes a new command:

image::ExportActivityDiagram.png[width="300"]

==== Design Considerations

**Aspect:** Implementation of Export mechanism +
**Alternative 1 (current choice):** Read the xml file and write into a csv file +
**Pros:** Easy and flexible to implement. +
**Cons:** Not standardised, therefore need to implement different method for different xml file format  +
**Alternative 2:** Create a XML Schema to standardised how the csv file will be like +
**Pros:** Since XML Schema has strict rules attached to it, we can just write a general export method for all kind of xml file format +
...
After a series of function calls, the *model.orderList()* method will eventually call *orderPersonList()* method
...
*orderPersonList()* method is able to order the existing person list by a maximum of two parameters.
It will order the list by the first parameter follow by the second parameter.
The implementation of the *orderPersonList()* is shown below:
...
This code was contributed by Esailija on StackOverflow, 2 Dec 2012.
